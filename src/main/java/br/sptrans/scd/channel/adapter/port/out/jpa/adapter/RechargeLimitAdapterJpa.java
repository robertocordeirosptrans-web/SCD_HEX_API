package br.sptrans.scd.channel.adapter.port.out.jpa.adapter;

import java.math.BigDecimal;
import java.sql.ResultSet;
import java.sql.SQLException;
import java.sql.Timestamp;
import java.time.LocalDateTime;
import java.util.List;
import java.util.Optional;

import org.springframework.jdbc.core.JdbcTemplate;
import org.springframework.jdbc.core.RowMapper;
import org.springframework.stereotype.Repository;

import br.sptrans.scd.auth.domain.User;
import br.sptrans.scd.channel.application.port.out.RechargeLimitRepository;
import br.sptrans.scd.channel.domain.ProductChannel;
import br.sptrans.scd.channel.domain.ProductChannelKey;
import br.sptrans.scd.channel.domain.RechargeLimit;
import br.sptrans.scd.channel.domain.RechargeLimitKey;
import br.sptrans.scd.channel.domain.SalesChannel;
import br.sptrans.scd.product.domain.Product;
import lombok.RequiredArgsConstructor;

@Repository
@RequiredArgsConstructor
public class RechargeLimitAdapterJpa implements RechargeLimitRepository {

    private final JdbcTemplate jdbc;

    private final RowMapper<RechargeLimit> rowMapper = (rs, rowNum) -> mapRow(rs);

    // -------------------------------------------------------------------------
    // SQL
    // -------------------------------------------------------------------------
    private static final String SQL_SELECT_BASE = """
            SELECT COD_CANAL, COD_PRODUTO, DT_INICIO_VALIDADE, DT_FIM_VALIDADE,
                   VL_MINIMO_RECARGA, VL_MAXIMO_RECARGA, VL_MAXIMO_SALDO,
                   COD_STATUS, DT_MANUTENCAO, ID_USUARIO
            FROM SPTRANSDBA.LIMITES_RECARGA
            """;

    private static final String SQL_INSERT = """
            INSERT INTO SPTRANSDBA.LIMITES_RECARGA (
                COD_CANAL, COD_PRODUTO, DT_INICIO_VALIDADE, DT_FIM_VALIDADE,
                VL_MINIMO_RECARGA, VL_MAXIMO_RECARGA, VL_MAXIMO_SALDO,
                COD_STATUS, DT_MANUTENCAO, ID_USUARIO
            ) VALUES (?,?,?,?,?,?,?,?,SYSDATE,?)
            """;

    private static final String SQL_UPDATE = """
            UPDATE SPTRANSDBA.LIMITES_RECARGA SET
                DT_INICIO_VALIDADE = ?,
                DT_FIM_VALIDADE    = ?,
                VL_MINIMO_RECARGA  = ?,
                VL_MAXIMO_RECARGA  = ?,
                VL_MAXIMO_SALDO    = ?,
                COD_STATUS         = ?,
                DT_MANUTENCAO      = SYSDATE,
                ID_USUARIO         = ?
            WHERE COD_CANAL = ? AND COD_PRODUTO = ?
            """;

    private static final String SQL_DELETE = """
            DELETE FROM SPTRANSDBA.LIMITES_RECARGA
            WHERE COD_CANAL = ? AND COD_PRODUTO = ?
            """;

    private static final String SQL_EXISTS = """
            SELECT COUNT(1) FROM SPTRANSDBA.LIMITES_RECARGA
            WHERE COD_CANAL = ? AND COD_PRODUTO = ?
            """;

    // -------------------------------------------------------------------------
    // Interface
    // -------------------------------------------------------------------------

    @Override
    public Optional<RechargeLimit> findById(RechargeLimitKey id) {
        List<RechargeLimit> result = jdbc.query(
                SQL_SELECT_BASE + "WHERE COD_CANAL = ? AND COD_PRODUTO = ?",
                rowMapper, id.getCodCanal(), id.getCodProduto());
        return result.isEmpty() ? Optional.empty() : Optional.of(result.get(0));
    }

    @Override
    public List<RechargeLimit> findAll() {
        return jdbc.query(SQL_SELECT_BASE, rowMapper);
    }

    @Override
    public List<RechargeLimit> findByCodCanal(String codCanal) {
        return jdbc.query(SQL_SELECT_BASE + "WHERE COD_CANAL = ?", rowMapper, codCanal);
    }

    @Override
    public List<RechargeLimit> findByCodProduto(String codProduto) {
        return jdbc.query(SQL_SELECT_BASE + "WHERE COD_PRODUTO = ?", rowMapper, codProduto);
    }

    @Override
    public RechargeLimit save(RechargeLimit entity) {
        RechargeLimitKey key = entity.getId();
        if (existsById(key)) {
            jdbc.update(SQL_UPDATE,
                    ts(entity.getDtInicioValidade()),
                    ts(entity.getDtFimValidade()),
                    entity.getVlMinimoRecarga(),
                    entity.getVlMaximoRecarga(),
                    entity.getVlMaximoSaldo(),
                    entity.getCodStatus(),
                    entity.getIdUsuarioCadastro() != null ? entity.getIdUsuarioCadastro().getIdUsuario() : null,
                    key.getCodCanal(),
                    key.getCodProduto());
        } else {
            jdbc.update(SQL_INSERT,
                    key.getCodCanal(),
                    key.getCodProduto(),
                    ts(entity.getDtInicioValidade()),
                    ts(entity.getDtFimValidade()),
                    entity.getVlMinimoRecarga(),
                    entity.getVlMaximoRecarga(),
                    entity.getVlMaximoSaldo(),
                    entity.getCodStatus(),
                    entity.getIdUsuarioCadastro() != null ? entity.getIdUsuarioCadastro().getIdUsuario() : null);
        }
        return findById(key).orElseThrow();
    }

    @Override
    public void deleteById(RechargeLimitKey id) {
        jdbc.update(SQL_DELETE, id.getCodCanal(), id.getCodProduto());
    }

    @Override
    public boolean existsById(RechargeLimitKey id) {
        Integer count = jdbc.queryForObject(SQL_EXISTS, Integer.class,
                id.getCodCanal(), id.getCodProduto());
        return count != null && count > 0;
    }

    // -------------------------------------------------------------------------
    // RowMapper
    // -------------------------------------------------------------------------

    private RechargeLimit mapRow(ResultSet rs) throws SQLException {
        try {
            String codCanal   = rs.getString("COD_CANAL");
            String codProduto = rs.getString("COD_PRODUTO");

            SalesChannel canal = new SalesChannel();
            setRef(canal, SalesChannel.class, "codCanal", codCanal);

            Product produto = new Product();
            setRef(produto, Product.class, "codProduto", codProduto);

            ProductChannel canalProduto = new ProductChannel();
            canalProduto.setId(new ProductChannelKey(codCanal, codProduto));

            User usuario = new User();
            usuario.setIdUsuario(rs.getLong("ID_USUARIO"));

            RechargeLimit rl = new RechargeLimit();
            rl.setId(new RechargeLimitKey(codCanal, codProduto));
            rl.setDtInicioValidade(fromTs(rs.getTimestamp("DT_INICIO_VALIDADE")));
            rl.setDtFimValidade(fromTs(rs.getTimestamp("DT_FIM_VALIDADE")));
            rl.setVlMinimoRecarga(rs.getObject("VL_MINIMO_RECARGA", BigDecimal.class));
            rl.setVlMaximoRecarga(rs.getObject("VL_MAXIMO_RECARGA", BigDecimal.class));
            rl.setVlMaximoSaldo(rs.getObject("VL_MAXIMO_SALDO", BigDecimal.class));
            rl.setCodStatus(rs.getString("COD_STATUS"));
            rl.setDtManutencao(fromTs(rs.getTimestamp("DT_MANUTENCAO")));
            rl.setIdUsuarioCadastro(usuario);
            rl.setCanalProduto(canalProduto);
            rl.setCanal(canal);
            rl.setProduto(produto);
            return rl;
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }

    // -------------------------------------------------------------------------
    // Helpers
    // -------------------------------------------------------------------------

    private static void setRef(Object target, Class<?> clazz, String fieldName, Object value)
            throws Exception {
        var field = clazz.getDeclaredField(fieldName);
        field.setAccessible(true);
        field.set(target, value);
    }

    private static Timestamp ts(LocalDateTime ldt) {
        return ldt != null ? Timestamp.valueOf(ldt) : null;
    }

    private static LocalDateTime fromTs(Timestamp ts) {
        return ts != null ? ts.toLocalDateTime() : null;
    }
}
