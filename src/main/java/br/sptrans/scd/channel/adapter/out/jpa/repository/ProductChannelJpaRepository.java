...
public interface ProductChannelJpaRepository extends JpaRepository<ProductChannelEntityJpa, ProductChannelKeyEntityJpa>,
        JpaSpecificationExecutor<ProductChannelEntityJpa> {
    ...
    @Query("""
                SELECT p FROM ProductChannelEntityJpa p
                WHERE p.id.codCanal = :codCanal
                  AND p.codStatus = :stCanaisProdutos
                  AND p.product.codStatus = :stProdutos
            """)
    List<ProductChannelEntityJpa> findAvailableProductsByChannel(
            @Param("codCanal") String codCanal,
            @Param("stCanaisProdutos") String stCanaisProdutos,
            @Param("stProdutos") String stProdutos);
